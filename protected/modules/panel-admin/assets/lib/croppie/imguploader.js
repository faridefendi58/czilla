let name="";
let modal,target;
let cropper,img_input,status,size,temp,dest_name;
let passurl="";
var Filter={};
...
$(".img-upload-input").on('change',function(event) {
    ...
    size=$(this).attr('size');
    temp=$(this).attr('temp');
    dest_name=$(this).attr('dest_file_name');
    ...
    if(file !=undefined)
    {
        ...
        let max_file_size = 500000;
        try {
            $(status).empty();
            img_input=$(this);
            if (file.size > max_file_size) {
                console.log("big file size "+ file.size);
                $(status).html("<div class=\"alert alert-danger\">Ukuran file terlalu besar. <small>Maksimum 500 kb.</small></div>");
...
$(".img-upload-btn").on('click',function(event) {
    ...
    cropper.croppie('result',{
        ...
        {
            $.ajax({
                data:{"image":response,"name":name,"dest_file_name":dest_name},
                url:passurl,
                type:'POST',
                dataType: "json",
            }).done(function(data)
